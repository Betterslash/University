...
public class PrgRunController implements Initializable {
    ...
    public void setOutList() {
        ...
        List<Value> outputLists = myController.getRepository().getPrgList().get(0).getOut().getList();
        if(!outputLists.isEmpty()) {
            for (Value e : outputLists) {
    ...
    public void setSymTableAndExeStack() {
        ...
        if(prgResult != null) {
            symTableList.addAll(prgResult.getTopOfSymStack().getRepresentation().entrySet());
