...
public class PrgRunController implements Initializable {
    ...
    @FXML
    TableColumn<Map.Entry<Integer, Integer>, String> lockId;
    @FXML
    TableColumn<Map.Entry<Integer, Integer>, String> lockAddress;
    @FXML
    ...
    public void setLockTable(){
        ...
        lockId.setCellValueFactory(cellData -> new ReadOnlyStringWrapper(Integer.toString(cellData.getValue().getKey())));
        lockAddress.setCellValueFactory(cellData -> new ReadOnlyStringWrapper(cellData.getValue().getValue().toString()));
        lockTableList.addAll(myController.getRepository().getPrgList().get(0).getLockTable().getRepresentation().entrySet());
        lockTable.refresh();
