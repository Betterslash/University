...
public class PrgListController implements Initializable {
    ...
    public void setUp() {
        ...
        params.add("a");
        params.add("b");
        ArrayList<Expression> callParams = new ArrayList<>();
        callParams.add(new VarExp("v"));
        callParams.add(new VarExp("w"));
        ArrayList<Expression> callParams2 = new ArrayList<>();
        callParams2.add(new ArithmExp('*',new VarExp("v"), new ValueExp(new IntValue(10))));
        callParams2.add(new VarExp("w"));

        IStmt ex0 = new CompStmt(new CompStmt(
                ...
                new CompStmt(new CompStmt(new VarDeclStmt("w", new IntType()), new AssignStmt("w", new ValueExp(new IntValue(5)))),
                        new CompStmt(new CompStmt(new CallStmt("sum",callParams2), new PrintStmt(new VarExp("v"))),
                                        new CompStmt(new ForkStmt(new CompStmt(new CallStmt("product", callParams),
                                               new ForkStmt(new CallStmt("sum", callParams)))),new NoStmt()))));
        ...
        prgState0.getGlobalTable().getRepresentation().put("sum", new MyPair<>(params, new CompStmt(new AssignStmt("v", new ArithmExp('+', new VarExp("a"), new VarExp("b"))), new PrintStmt(new VarExp("v")))));
        prgState0.getGlobalTable().getRepresentation().put("product", new MyPair<>(params,new CompStmt(new AssignStmt("v", new ArithmExp('*', new VarExp("a"), new VarExp("b"))), new PrintStmt(new VarExp("v")))));
    ...
    private void checker(IStmt ex0, IStmt ex1, IStmt ex2) {
        try {
