...
public class PrgRunController implements Initializable {
    ...
    @FXML
    TableView<Map.Entry<Integer, MyPair<Integer, List<Integer>>>> countSemaphoreTable;
    @FXML
    TableColumn<Map.Entry<Integer, MyPair<Integer, List<Integer>>>,String> csID;
    @FXML
    TableColumn<Map.Entry<Integer, MyPair<Integer, List<Integer>>>,String> csValue;
    @FXML
    ...
    public void setCSemTable() {
        ObservableList<Map.Entry<Integer,MyPair<Integer, List<Integer>>>> CSemList = FXCollections.observableArrayList();
        csID.setCellValueFactory(cellData -> new ReadOnlyStringWrapper(Integer.toString(cellData.getValue().getKey())));
        csValue.setCellValueFactory(cellData -> new ReadOnlyStringWrapper(cellData.getValue().getValue().toString()));
        CSemList.addAll(myController.getRepository().getPrgList().get(0).getSemaphoreTable().getRepresentation().entrySet());
        countSemaphoreTable.refresh();
        countSemaphoreTable.setItems(CSemList);
    }
